...
const TOC_SECTIONS = [
  { id: 'penelitian',  label: 'PENELITIAN',               indent: false },
  { id: 'publikasi',   label: 'PUBLIKASI',                indent: false },
  { id: 'peer',        label: 'Artikel peer-reviewed',    indent: true  },
  { id: 'chapters',    label: 'Bab dalam buku',           indent: true  },
  { id: 'reviews',     label: 'Ulasan buku & konferensi', indent: true  },
  { id: 'public',      label: 'Tulisan publik',           indent: true  },
  { id: 'pengajaran',  label: 'PENGAJARAN',               indent: false },
  { id: 'lectured',    label: 'Mata kuliah diampu',       indent: true  },
  { id: 'guest',       label: 'Seminar & workshop tamu',  indent: true  },
]

const PUB_CATEGORIES = [
  { key: 'all',      label: 'Semua' },
  { key: 'peer',     label: 'Artikel' },
  { key: 'chapters', label: 'Bab' },
...
export default function PengalamanClient() {
  ...
  const [activeId,  setActiveId]  = useState('penelitian')
    ...
    <>
      ...
      <div style={{ display: 'flex', minHeight: '100vh' }}>
        ...
        <aside className="work-sidebar">
          <h1 className="work-sidebar__heading">PENGALAMAN</h1>
        ...
        <main style={{ flex: 1, minWidth: 0 }}>
          ...
          >
            <span>
              {TOC_SECTIONS.find(s => s.id === activeId)?.label ?? 'PENGALAMAN'}
            </span>
          ...
          {/* PENELITIAN */}
          <section>
            <div id="penelitian" style={{ scrollMarginTop: '80px' }}>
              <SectionBanner label="PENELITIAN" color="#c94f35" />
            </div>
          ...
          {/* PUBLIKASI */}
          <section>
            <div id="publikasi" style={{ scrollMarginTop: '80px' }}>
              <SectionBanner label="PUBLIKASI" color="#8a7d3a" />
            </div>
            ...
            <div className="content-pad">
              {showAllPubs ? (
                <>
                  <SubBanner id="peer"     label="Artikel peer-reviewed" />
                  {publications.peer.map((item, i) => <PublicationItem key={i} item={item} />)}

                  <SubBanner id="chapters" label="Bab dalam buku yang diedit" />
                  {publications.chapters.map((item, i) => <PublicationItem key={i} item={item} />)}

                  <SubBanner id="reviews"  label="Ulasan buku & konferensi" />
                  {publications.reviews.map((item, i) => <PublicationItem key={i} item={item} />)}

                  <SubBanner id="public"   label="Tulisan publik" />
          ...
          {/* PENGAJARAN */}
          <section>
            <div id="pengajaran" style={{ scrollMarginTop: '80px' }}>
              <SectionBanner label="PENGAJARAN" color="#6b1f3a" />
            </div>
            {teaching.lectured.map((item, i) => (
              <ResearchCard key={item.id} item={item} onClick={setModal} colorIndex={i + 2} />
            ))}
            <div className="content-pad">
              <SubBanner id="lectured" label="Mata kuliah diampu" />
              <SubBanner id="guest"    label="Seminar dan workshop tamu" />
